	...
	for i in range(len(input)):
		...
		for i, img in enumerate(images):
			...
			if average < (255 * MAX_BACKGROUND_PERCENT):
				...
				filename = "./output/{0}-{1}_{2}.jpg"
				cv.imwrite(filename.format(shape["centerx"], shape["centery"], "shape"), shape["mask"])
				cv.imwrite(filename.format(shape["centerx"], shape["centery"], "image"), shape["image"])
				cv.imwrite(filename.format(shape["centerx"], shape["centery"], "letter"), shape["letter"])
